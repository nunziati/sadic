...
default_quantizer_kwargs = {"rho_steps_number": , "theta_steps_number": 360, "phi_steps_number": 180}
...
def find_candidate_max_radius_points(multisphere: Multisphere, quantizer_arg: SphericalQuantizer | None = None, min_radius: float = 1.52, multiplier: float = 2) -> tuple[NDArray[np.float32], float]:
...
def find_max_radius_point(multisphere: Multisphere, quantizer_arg: SphericalQuantizer | None = None, min_radius: float = 1.52, multiplier: float = 2, bisection_threshold: float = 1) -> tuple[NDArray[np.int32], int]:
    quantizer: SphericalQuantizer = default_quantizer_class(**default_quantizer_kwargs) if quantizer_arg is None else quantizer_arg

    candidate_max_radius_points, max_radius = find_candidate_max_radius_points(multisphere, quantizer, min_radius, multiplier)
    
    max_radii: NDArray[np.int32] = np.empty(candidate_max_radius_points.shape[0], dtype=np.int32)
